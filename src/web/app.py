"""
Flask Web Application — Dashboard for oil spill detection, drift
visualisation, and vessel attribution.

Routes:
  GET  /                     — Main dashboard
  POST /api/detect           — Upload image for detection
  POST /api/demo             — Run full demo pipeline
  GET  /api/result/<run_id>  — Retrieve pipeline result
  GET  /api/trajectory/<run_id> — Get trajectory GeoJSON
...
# In-memory result store (use a database in production)
...
def create_app(config: Optional[dict] = None) -> Flask:
    ...
    app.config["SECRET_KEY"] = "oil-spill-detection-dev-key"
    ...
    def run_demo():
        """Run the full demo pipeline with synthetic data."""
        ...
        pipeline = PipelineOrchestrator(PipelineConfig(
            output_dir=str(Path(app.static_folder) / "results"),
    ...
    def detect_image():
        """Upload and detect oil spills in an image."""
    ...
    def get_result(run_id):
        """Retrieve a pipeline result by run ID."""
    ...
    def get_trajectory(run_id):
        """Get trajectory data as GeoJSON for map visualisation."""
        result = _results_store.get(run_id)
        if not result:
            return jsonify({"error": "Result not found"}), 404
